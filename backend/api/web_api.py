...
_READYKIDS_STARS_ADVENTURE_BASE_EN = "Stars Adventure (ages 6-8, English)"
...
def _stars_adventure_specs_en() -> list[dict]:
    from backend.core.adventure_quiz import adventure_chunk

    return [
        {
            "question": (
                "I will ask you short questions. If you answer correctly you will earn a star. "
                "Are you ready?"
            ),
            "options": {"A": "-", "B": "-", "C": "-"},
            "correct": "A",
        ...
        {
            "question": (
                "Stage one: listening attention. "
                "Listen carefully. You heard: cat, dog, apple. "
                "Which animal barks?"
            ),
            "options": {"A": "cat", "B": "dog", "C": "apple"},
            "correct": "B",
            "chunk_text": adventure_chunk(
                1,
                "question",
                counts_star=True,
                success_feedback="Excellent! You earned a star.",
            ),
        },
        {
            "question": (
                "Stage two: short-term memory. "
                "Remember these numbers: 3, 7, 2. "
            ...
            "chunk_text": adventure_chunk(
                ...
                "question",
                counts_star=True,
                success_feedback="Great! Your memory is strong. A star.",
            ),
        },
        {
            "question": (
                "Stage three: find the odd one out. "
                "Which word does not belong in the group?"
            ),
            "options": {"A": "car", "B": "bus", "C": "banana"},
            "correct": "C",
            "chunk_text": adventure_chunk(
                3,
                "question",
                counts_star=True,
                success_feedback="Well done! You spotted the odd one. A star.",
            ),
        },
        {
            "question": (
            ...
            "chunk_text": adventure_chunk(
                ...
                "question",
                counts_star=True,
                success_feedback="Well done! A star.",
            ),
        },
    ]
...
def _create_ready_kids_stars_adventure_program_en(db, specialist_id: int) -> TrainingProgramModel:
    """Ready scripted Stars Adventure for English Alexa (ages 6–8)."""
    return _create_ready_kids_program_from_specs(
        db, specialist_id, _READYKIDS_STARS_ADVENTURE_BASE_EN, _stars_adventure_specs_en()
    )
...
def create_web_api() -> Flask:
    ...
    @app.route("/api/specialists/library/demo-stars-adventure-en", methods=["POST"])
    def create_specialist_library_demo_stars_adventure_en():
        sid = _get_specialist_id()
        if not sid:
            return _auth_required()
        with get_db() as db:
            ...
            if sub.get("library_frozen"):
                return _subscription_error_response(sub, "subscription_library_frozen")
            item = _create_ready_kids_stars_adventure_program_en(db, sid)
            db.commit()
            t_repo = TrainingProgramRepository(db)
            return jsonify(t_repo._to_dict(item)), 201
    ...
    @app.route("/api/parents/library/demo-stars-adventure-en", methods=["POST"])
    def create_parent_library_demo_stars_adventure_en():
        pid = _get_parent_id()
        if not pid:
            return _auth_required()
        with get_db() as db:
